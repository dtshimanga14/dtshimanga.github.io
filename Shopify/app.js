...
async function run() {
  try {
    ...
    products = (await productsList.find({}).toArray()).map(prepare);
...
app.post("/addPicture", upload.single("file"),async (req, res,err) => {
  ...
  .get('/board',(req,res) => {
    ...
    //res.render("Board",{ products : products, n : 2 });
    ...
    let id = (req.params.id).trim();
	  gfs.delete(new mongoose.Types.ObjectId(id), 
      (err, data) => {
        if (err) 
          return res.status(404).json({ err: err.message });
        console.log("product is deleted "+req.params.id);
    });
    console.log(id);
	}).get('/',(req,res) => {
...
.get("/login",(req,res) => {
    ...
    let sum = items ? items.map(res => parseInt(res.price))
                .reduce((acc,item) => (acc + item)) : 0;
                
    res.render("cart",{ items : items ? items : [], n : n, sum : sum });
