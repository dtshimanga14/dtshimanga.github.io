...
import Loading from './Loading.js';
	...
	onFormSubmit = (evt) =>{
		...
		let linkApi = 'http://localhost:8000/avatar/'+ownerId;
		fetch(linkApi,{
			method : 'post',
			body: formData,
			mode : "no-cors"
						...
						<div className="sub-sect-cv-zone">
							...
							<div  className="cv-flex-box">
								...
								<div className="flex-frame-photo-cv">
									<div>
										<img className="photo-cv" src={this.state.uploadFileUrl !=="" ? this.state.uploadFileUrl : `http://localhost:8000/image/${users.avatar}`}  height="300px" width="300px"/>
									</div>
									<div >
									...
									<div className="">
										<form  method="post" enctype="multipart/form-data" onSubmit={this.onFormSubmit}>
											<input type="file" className="" name="file" id="avatarFile" onChange = {(e)=>this.previewImage(e)}/>
